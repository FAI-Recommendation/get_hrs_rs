"""
models/combigcn.py — CombiGCN (PyG)
=====================================
Hỗ trợ 4 chế độ:
  sim_type=none      → LightGCN thuần (CF only)
  sim_type=tfidf     → CombiGCN + TF-IDF item similarity
  sim_type=img_only  → CombiGCN + image item similarity
  sim_type=multimodal→ CombiGCN + multimodal item similarity
"""
...
class CombiGCN(nn.Module):
    ...
    Khi similarity_adj = None (LightGCN thuần):
        Mỗi layer:  ego_embed = norm_adj @ ego_embed

    Khi similarity_adj != None (CombiGCN):
        Mỗi layer:
            ...
            5. item_next = item_interaction + item_similar
